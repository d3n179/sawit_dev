<?PHP
class cetakLapIncentiveKaryawanPdf extends MainConf
{
	...
	public function onLoad($param)
	{
	    ...
	    $pdf->Cell(0,5,'LAPORAN INCENTIVE KARYAWAN','0',0,'C');
		$pdf->Ln(4);
		...
		$pdf->SetWidths(array(80,50,30,35));
		$pdf->Row(array('Nama Karyawan','Jabatan','Golongan','Jumlah Incentive'));
		//$pdf->Ln(1);
		$pdf->SetFont('Arial','',8);
		$pdf->SetAligns(array('L','L','L','R'));
		$session=new THttpSession;
		$session->open();
		$sql = $session['cetakLapIncentiveKaryawanSql'];
		...
		{		
			$pdf->Row(array($row['nama'],
							$row['jabatan'],
							$row['golongan'],
							number_format($row['jml_incentive'])));
			$i++;
		}
