<?php
class cetakJadwalKaryawanXls extends XlsGen
{
	...
	{		
		...
		$baris=0;
		$kolom=0;
		$worksheet1->write_string($baris,0,'JADWAL KARYAWAN' ,$frmtTitleLeft);
		...
		foreach($arrKaryawan as $rowKaryawan)
		{
			...
			{
				$date = $thn."-".$bln."-".$i;
				$JadwalRecord = JadwalRecord::finder()->find('idkaryawan = ? AND tanggal = ?',$rowKaryawan['id'],$date);
				if($JadwalRecord)
				{
					$ShiftSettingRecord = ShiftSettingRecord::finder()->findByPk($JadwalRecord->shif);
					$shif = $ShiftSettingRecord->datang.' - '.$ShiftSettingRecord->pulang;
				}
				else
					$shif = '';
						
				$worksheet1->write_string($baris,$kolom,$shif,$center);	$kolom++;
				$i++;
			}
