<?PHP
class cetakSanksiTransactionPdf extends MainConf
{
	...
	{	
		...
		$pdf->Cell(0,5,'SANKSI REPORT','0',0,'C');
		$pdf->Ln(5);
		$pdf->SetAligns(array('C','C','C'));
		$pdf->SetWidths(array(60,35,100));
		$pdf->Row(array('Nama Karyawan','Tgl. Sanksi','Sanksi'));
		//$pdf->Ln(1);
		$pdf->SetFont('Arial','',8);
		...
		{		
			$pdf->Row(array($row['karyawan'],
							$this->ConvertDate($row['tgl_sanksi'],'3'),
							$row['sanksi']));
			$i++;
		}
