<?PHP
class cetakLaporanGajiHarianKaryawanPdf extends MainConf
{
	...
	public function onLoad($param)
	{
		...
		elseif($periode == '2')
		{
			...
			$tgl1 = $this->ConvertDate($tgl1,'3');
			$tgl2 = $this->ConvertDate($tgl2,'3');
	    ...
	    $pdf->Cell(0,5,'LAPORAN GAJI HARIAN KARYAWAN','0',0,'C');
		$pdf->Ln(4);
			...
			{	
				...
				$totalGajiKotorLembur = 0;
					...
					if($periode == '0')
					{
						...
						$gaji_pokok = $GolonganKaryawanRecord->gaji_pokok;
						$tunjangan_natura = $KaryawanRecord->tunjangan_natura;
						$jml_incentive = $IncentiveRecord->jml_incentive;
						$tunjangan_jabatan = $JabatanRecord->tunjangan_jabatan;
						$tunjangan_komunikasi = $JabatanRecord->tunjangan_komunikasi;
						$premi_karyawan = $JabatanRecord->premi_karyawan;
					}
					elseif($periode == '1')
					{
						...
						$gaji_pokok = $GolonganKaryawanRecord->gaji_pokok * 12;
						$tunjangan_natura = $KaryawanRecord->tunjangan_natura * 12;
						$jml_incentive = $IncentiveRecord['jml_incentive'];
						$tunjangan_jabatan = $JabatanRecord->tunjangan_jabatan * 12;
						$tunjangan_komunikasi = $JabatanRecord->tunjangan_komunikasi * 12;
						$premi_karyawan = $JabatanRecord->premi_karyawan * 12;
					...
					elseif($periode == '2')
					{
						...
						$totalGajiKotor += ($GolonganKaryawanRecord->gaji_pokok / 25) * $dateDiff;
						$totalGajiKotor += ($KaryawanRecord->tunjangan_natura / 25) * $dateDiff;
						$totalGajiKotor += $IncentiveRecord->jml_incentive;
						$totalGajiKotor += ($JabatanRecord->tunjangan_jabatan / 25) * $dateDiff;
						$totalGajiKotor += ($JabatanRecord->tunjangan_komunikasi / 25) * $dateDiff;
						$totalGajiKotor += ($JabatanRecord->premi_karyawan / 25) * $dateDiff;
						
						$gaji_pokok = ($GolonganKaryawanRecord->gaji_pokok / 25) * $dateDiff;
						$tunjangan_natura = ($KaryawanRecord->tunjangan_natura / 25) * $dateDiff;
						$jml_incentive = $IncentiveRecord->jml_incentive;
						$tunjangan_jabatan = ($JabatanRecord->tunjangan_jabatan / 25) * $dateDiff;
						$tunjangan_komunikasi = ($JabatanRecord->tunjangan_komunikasi / 25) * $dateDiff;
						$premi_karyawan = ($JabatanRecord->premi_karyawan / 25) * $dateDiff;
						
					}
					elseif($periode == '3')
					{
						$tgl2 = $this->ConvertDate($harian,'2');
					...
					if($periode == '0')
					{
						...
						$bpjsTenagaKerja = $bpjsTenagaKerja;
					}
					elseif($periode == '1')
					{
						$bpjsKesehatan = $bpjsKesehatan * 12;
						$bpjsTenagaKerja = $bpjsTenagaKerja * 12;
					}
					elseif($periode == '2')
					{
						$mingguan = explode("-",$mingguan);
					...
					$totalGajiPokokCol += $gaji_pokok;
					$totalTunjanganNaturaCol += $tunjangan_natura;
					$totalJmlIncentiveCol += $jml_incentive;
					$totalTunjanganJabatanCol += $tunjangan_jabatan;
					$totalTunjanganKomunikasiCol += $tunjangan_komunikasi;
					$totalPremiKaryawanCol += $premi_karyawan;
					...
					$pdf->Row(array($no,
									...
									number_format($tunjangan_natura,0,'.',','),
									number_format($jml_incentive,0,'.',','),
									number_format($tunjangan_jabatan,0,'.',','),
									number_format($tunjangan_komunikasi,0,'.',','),
									number_format($premi_karyawan,0,'.',','),
