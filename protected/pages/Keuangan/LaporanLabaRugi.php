<?PHP
class LaporanLabaRugi extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans = "SELECT 
						tbt_laba_rugi.id,
						tbt_laba_rugi.tgl_transaksi,
						tbt_laba_rugi.jns_transaksi,
						tbt_laba_rugi.no_transaksi,
						tbt_laba_rugi.keterangan,
						tbt_laba_rugi.jumlah_transaksi
					FROM 
						tbt_laba_rugi
					WHERE
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_laba_rugi.tgl_transaksi) = '$bulan' AND YEAR(tbt_laba_rugi.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_laba_rugi.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_laba_rugi.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY tbt_laba_rugi.jns_transaksi ASC,tbt_laba_rugi.tgl_transaksi ASC ";
		...
		if($arrTrans)
		{
			foreach($arrTrans as $row)
			{
				...
				$tblBody .= '<td>'.number_format($row['jumlah_transaksi'],2,".",",").'</td>';	
				$tblBody .= '</tr>';
			}
		}
		else
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapLabaRugiSql'] = $this->getViewState('sql');
				
		$url = "index.php?page=Keuangan.cetakLapLabaRugiPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
