<?PHP
class LaporanJurnalUmum extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_umum.id,
						tbt_jurnal_umum.tgl_transaksi,
						tbt_jurnal_umum.jns_transaksi,
						tbt_jurnal_umum.no_transaksi,
						tbt_jurnal_umum.keterangan,
						tbt_jurnal_umum.jumlah_saldo
					FROM 
						tbt_jurnal_umum
					WHERE
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_umum.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_umum.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_umum.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_umum.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		...
		if($arrTrans)
		{
			foreach($arrTrans as $row)
			{
				...
				{
					$tblBody .= '<td>'.number_format($row['jumlah_saldo'],2,".",",").'</td>';
					$tblBody .= '<td></td>';
				}
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalUmumSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalUmumPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
