<?PHP
class cetakLapJurnalPenyesuaianPdf extends MainConf
{
	...
	public function onLoad($param)
	{
		...
		$pdf->Cell(0,5,'LAPORAN JURNAL PENYESUAIAN','0',0,'C');
		$pdf->Ln(4);
		...
		$pdf->SetWidths(array(35,100,30,30));
		$pdf->Row(array('TGL TRANSAKSI','KETERANGAN', 'DEBET','KREDIT'));
		//$pdf->Ln(1);
		//$pdf->SetFont('Arial','',8);
		//$pdf->SetAligns(array('C','L','L','L','L','R','R','R','R','R'));
		$session=new THttpSession;
		$session->open();
		$sql = $session['cetakLapJurnalPenyesuaianSql'];
		...
		foreach($arrData as $row)
		{
			...
			{
				$pdf->Row(array($this->ConvertDate($row['tgl_transaksi'],'3'),$row['keterangan'],number_format($row['jumlah_saldo'],2,".",","),''));
				$totalDebet += $row['jumlah_saldo'];
			}
			elseif($row['jns_transaksi'] == '1')
			{
				$pdf->Row(array($this->ConvertDate($row['tgl_transaksi'],'3'),chr(32).chr(32).chr(32).chr(32).chr(32).chr(32).chr(32).$row['keterangan'],'',number_format($row['jumlah_saldo'],2,".",",")));
