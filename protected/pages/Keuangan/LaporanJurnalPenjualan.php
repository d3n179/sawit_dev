<?PHP
class LaporanJurnalPenjualan extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_penjualan.id,
						tbt_jurnal_penjualan.tgl_transaksi,
						tbt_jurnal_penjualan.jns_transaksi,
						tbt_jurnal_penjualan.no_transaksi,
						tbt_jurnal_penjualan.keterangan,
						tbt_jurnal_penjualan.syarat,
						tbt_jurnal_penjualan.ref,
						tbt_jurnal_penjualan.jumlah
					FROM 
						tbt_jurnal_penjualan
					WHERE
						tbt_jurnal_penjualan.deleted ='0' ";
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_penjualan.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_penjualan.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_penjualan.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_penjualan.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY
							tbt_jurnal_penjualan.tgl_transaksi ASC ";
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalPenjualanSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalPenjualanPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
