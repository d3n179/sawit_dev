<?PHP
class LaporanJurnalPenerimaanKas extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_penerimaan_kas.id,
						tbt_jurnal_penerimaan_kas.tgl_transaksi,
						tbt_jurnal_penerimaan_kas.jns_transaksi,
						tbt_jurnal_penerimaan_kas.no_transaksi,
						tbt_jurnal_penerimaan_kas.keterangan,
						tbt_jurnal_penerimaan_kas.nama_akun,
						tbt_jurnal_penerimaan_kas.ref,
						tbt_jurnal_penerimaan_kas.jumlah,
						tbt_jurnal_penerimaan_kas.potongan
					FROM 
						tbt_jurnal_penerimaan_kas
					WHERE
						tbt_jurnal_penerimaan_kas.deleted ='0' ";
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_penerimaan_kas.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_penerimaan_kas.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_penerimaan_kas.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_penerimaan_kas.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY
							tbt_jurnal_penerimaan_kas.tgl_transaksi ASC ";
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalPenerimaanKasSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalPenerimaanKasPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
