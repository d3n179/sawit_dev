<?PHP
class cetakLapJurnalPenerimaanKasPdf extends MainConf
{
	...
	public function onLoad($param)
	{
		...
		$pdf->Cell(0,5,'LAPORAN JURNAL PENERIMAAN KAS','0',0,'C');
		$pdf->Ln(4);
		...
		$session->open();
		$sql = $session['cetakLapJurnalPenerimaanKasSql'];
		...
		foreach($arrData as $row)
		{
			...
			$totalPotPenjualan += $row['potongan'];	
			$totalKas += $row['jumlah'];	
		...
		$pdf->Row(array('Jumlah',
						...
						number_format($totalPotPenjualan,2,".",","),
						number_format($totalPiutangdagang,2,".",","),
						number_format($totalPenjualan,2,".",","),
