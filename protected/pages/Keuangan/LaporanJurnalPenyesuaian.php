<?PHP
class LaporanJurnalPenyesuaian extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_penyesuaian.id,
						tbt_jurnal_penyesuaian.tgl_transaksi,
						tbt_jurnal_penyesuaian.jns_transaksi,
						tbt_jurnal_penyesuaian.keterangan,
						tbt_jurnal_penyesuaian.jumlah_saldo
					FROM 
						tbt_jurnal_penyesuaian
					WHERE
						tbt_jurnal_penyesuaian.deleted ='0' ";
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_penyesuaian.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_penyesuaian.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_penyesuaian.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_penyesuaian.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY
							tbt_jurnal_penyesuaian.id ASC ";
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalPenyesuaianSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalPenyesuaianPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
