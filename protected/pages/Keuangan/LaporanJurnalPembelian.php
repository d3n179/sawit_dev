<?PHP
class LaporanJurnalPembelian extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_pembelian.id,
						tbt_jurnal_pembelian.tgl_transaksi,
						tbt_jurnal_pembelian.jns_transaksi,
						tbt_jurnal_pembelian.no_transaksi,
						tbt_jurnal_pembelian.keterangan,
						tbt_jurnal_pembelian.nama_akun,
						tbt_jurnal_pembelian.ref,
						tbt_jurnal_pembelian.jumlah
					FROM 
						tbt_jurnal_pembelian
					WHERE
						tbt_jurnal_pembelian.deleted ='0' ";
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_pembelian.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_pembelian.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_pembelian.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_pembelian.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY
							tbt_jurnal_pembelian.tgl_transaksi ASC ";
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalPembelianSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalPembelianPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
