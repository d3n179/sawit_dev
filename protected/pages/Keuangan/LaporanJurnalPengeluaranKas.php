<?PHP
class LaporanJurnalPengeluaranKas extends MainConf
{
	...
	public function cariBtnClicked($sender,$param)
	{
		...
		$sqlTrans ="SELECT 
						tbt_jurnal_pengeluaran_kas.id,
						tbt_jurnal_pengeluaran_kas.tgl_transaksi,
						tbt_jurnal_pengeluaran_kas.jns_transaksi,
						tbt_jurnal_pengeluaran_kas.no_transaksi,
						tbt_jurnal_pengeluaran_kas.keterangan,
						tbt_jurnal_pengeluaran_kas.nama_akun,
						tbt_jurnal_pengeluaran_kas.ref,
						tbt_jurnal_pengeluaran_kas.jumlah,
						tbt_jurnal_pengeluaran_kas.potongan
					FROM 
						tbt_jurnal_pengeluaran_kas
					WHERE
						tbt_jurnal_pengeluaran_kas.deleted ='0' ";
		...
		if($periode == '0')
		{
			...
			if($bulan != '' && $tahun != '')
			{
				$sqlTrans .="AND MONTH(tbt_jurnal_pengeluaran_kas.tgl_transaksi) = '$bulan' AND YEAR(tbt_jurnal_pengeluaran_kas.tgl_transaksi) = '$tahun' ";
			}
		}
		elseif($periode == '1')
		{
			...
			if($tahun != '')
			{
				$sqlTrans .="AND YEAR(tbt_jurnal_pengeluaran_kas.tgl_transaksi) = '$tahun' ";
			}
		}				
		elseif($periode == '2')
		{
			...
			if($mingguan != '')
			{
				...
				$sqlTrans .="AND tbt_jurnal_pengeluaran_kas.tgl_transaksi BETWEEN '$tgl1' AND '$tgl2' ";
			}
		}	
		
		$sqlTrans .=" ORDER BY
							tbt_jurnal_pengeluaran_kas.tgl_transaksi ASC ";
		...
		if($arrTrans)
		{
			foreach($arrTrans as $row)
			{
				...
				if($row['jns_transaksi'] == '3')
					$tblBody .= '<td>'.$row['nama_akun'].'</td>';
				else
					$tblBody .= '<td>'.$row['keterangan'].'</td>';
					
				$tblBody .= '<td>'.$row['ref'].'</td>';
				if($row['jns_transaksi'] == '1')
				{
					
					$tblBody .= '<td>'.number_format($row['jumlah'],2,".",",").'</td>';
					$tblBody .= '<td></td>';
					$tblBody .= '<td></td>';
				}
	...
	public function cetakLapKartuStok()
	{
		...
		//{
			...
			$session->open();
			$session['cetakLapJurnalPengeluaranKasSql'] = $this->getViewState('sql');
		
				
				$url = "index.php?page=Keuangan.cetakLapJurnalPengeluaranKasPdf&periode=".$this->Periode->SelectedValue."&bln=".$this->DDBulan->SelectedValue."&thn=".$this->DDTahun->SelectedValue."&mingguan=".$this->mingguan->Text;
